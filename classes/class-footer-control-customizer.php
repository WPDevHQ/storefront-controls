...
class Footer_Control_Customizer {
	/**
	...
	public function customizer_setup ( $wp_customize ) {
		if ( has_action( 'storefront_footer' ) ) {
		$wp_customize->add_section( 'footer_control', array(
			'title'          => __( 'Footer Control', 'storefront-controls' ),
			'priority'       => 75,
		) );
		...
		include_once( plugin_dir_path( __FILE__ ) . 'class-footer-control-customizer-control.php' );

		$theme = wp_get_theme();

		$wp_customize->add_control( new Footer_Control_Customizer_Control( $wp_customize, 'footer_control', array(
			'description'       => sprintf( __( 'Re-order the footer components in %s.<br /><br /> Please note that the Handheld Footer Bar can only be enable/disabled - re-odering will not have effect on it!', 'storefront-controls' ), $theme->__get( 'Name' ) ),
	...
	public function enqueue_scripts() {
		wp_enqueue_script( Storefront_Controls()->footer_token . '-sortables', esc_url( Storefront_Controls()->plugin_url . 'assets/js/footer.js' ), array( 'jquery', 'jquery-ui-sortable' ), Storefront_Controls()->version );
	}
	...
	public function enqueue_styles() {
		wp_enqueue_style( Storefront_Controls()->footer_token . '-customizer',  esc_url( Storefront_Controls()->plugin_url . 'assets/css/footer.css' ), '', Storefront_Controls()->version );
	}
	...
	*/
	...
	private function _get_footer_hooked_functions () {
		...
		if ( isset( $wp_filter[Storefront_Controls()->footer_hook] ) && 0 < count( $wp_filter[Storefront_Controls()->footer_hook] ) ) {
			foreach ( $wp_filter[Storefront_Controls()->footer_hook] as $k => $v ) {
				if ( is_array( $v ) ) {
					foreach ( $v as $i => $j ) {
	...
	private function _footer_format_title ( $key ) {
		$prefix = (string)apply_filters( 'footer_control_prefix', 'woo_display_' );
		$title = $key;
